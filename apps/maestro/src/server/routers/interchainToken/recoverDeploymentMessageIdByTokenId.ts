...
export const recoverDeploymentMessageIdByTokenId = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (!token.deploymentMessageId) {
      ...
      if (results.length === 0 && token.kind === "interchain") {
        // await postgres.deleteRemoteInterchainTokensByTokenId(input.tokenId);
        // await postgres.deleteInterchainTokenByTokenId(input.tokenId);

        console.log("deleted token records", input.tokenId);
      ...
      if (results.length) {
        ...
        console.log("updated token", input.tokenId, `${results[0].txHash}-0`);
        ...
        console.log("updated remote tokens", input.tokenId);
