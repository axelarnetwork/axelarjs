...
vi.mock("./cosmosSigner/signer", () => {
  ...
  const getCosmosSigner = vi.fn(() => ({
    signAndBroadcast: vi.fn().mockImplementation(mockBroadcastSuccess),
  }));
  return {
    getCosmosSigner,
  };
...
describe("addGas", () => {
  test("broadcast an IBC transfer", async () => {
    const txHash = `6118C285B0C7A139C5636184BECBF8C201FF36B61F44060B82EFE4C535084D9C`;
    const token = {
      denom:
    ...
    const sendOptions: SendOptions = {
      ...
      addressPrefix: "osmo",
      cosmos_mnemonic: process.env["COSMOS_WALLET_MNEMONIC"] as string,
    };
