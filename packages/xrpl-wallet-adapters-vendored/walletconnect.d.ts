import type { WalletConnectModalConfig } from '@walletconnect/modal';
import { type XRPLBaseWallet, XRPLWalletAccount } from './base/';
import type { StandardConnectFeature, StandardDisconnectFeature, StandardEventsFeature, XRPLIdentifierString, XRPLSignAndSubmitTransactionFeature, XRPLSignTransactionFeature } from './core';
export declare enum DEFAULT_XRPL_METHODS {
    XRPL_SIGN_TRANSACTION = "xrpl_signTransaction",
    XRPL_SIGN_TRANSACTION_FOR = "xrpl_signTransactionFor"
}
interface WalletConnectWalletProps {
    projectId: string;
    metadata?: {
        name: string;
        description: string;
        url: string;
        icons: string[];
    };
    networks: XRPLIdentifierString[];
    desktopWallets: WalletConnectModalConfig['desktopWallets'];
    mobileWallets: WalletConnectModalConfig['mobileWallets'];
}
export declare class WalletConnectWallet implements XRPLBaseWallet {
    #private;
    get version(): "1.0.0";
    get name(): string;
    get icon(): "data:image/svg+xml;base64,";
    get chains(): XRPLIdentifierString[];
    get features(): StandardConnectFeature & StandardEventsFeature & StandardDisconnectFeature & XRPLSignTransactionFeature & XRPLSignAndSubmitTransactionFeature;
    get accounts(): XRPLWalletAccount[];
    constructor({ projectId, metadata, networks, desktopWallets, mobileWallets }: WalletConnectWalletProps);
}
export {};
