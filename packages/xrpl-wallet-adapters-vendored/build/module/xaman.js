var __classPrivateFieldGet = (this && this.__classPrivateFieldGet) || function (receiver, state, kind, f) {
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a getter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
    return kind === "m" ? f : kind === "a" ? f.call(receiver) : f ? f.value : state.get(receiver);
};
var __classPrivateFieldSet = (this && this.__classPrivateFieldSet) || function (receiver, state, value, kind, f) {
    if (kind === "m") throw new TypeError("Private method is not writable");
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a setter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot write private member to an object whose class did not declare it");
    return (kind === "a" ? f.call(receiver, value) : f ? f.value = value : state.set(receiver, value)), value;
};
var _XamanWallet_instances, _XamanWallet_name, _XamanWallet_icon, _XamanWallet_accounts, _XamanWallet_client, _XamanWallet_sdk, _XamanWallet_listeners, _XamanWallet_connect, _XamanWallet_disconnect, _XamanWallet_signTransaction, _XamanWallet_signAndSubmitTransaction, _XamanWallet_on, _XamanWallet_emit, _XamanWallet_off, _XamanWallet_getXamanNetworkNameFromChain;
import { XRPLWalletAccount } from './base';
import { XAHAU_MAINNET, XAHAU_TESTNET, XRPL_DEVNET, XRPL_MAINNET, XRPL_TESTNET, } from './core';
import { Client } from 'xrpl';
import { XummPkce } from 'xumm-oauth2-pkce';
export class XamanWallet {
    get version() {
        return '1.0.0';
    }
    get name() {
        return __classPrivateFieldGet(this, _XamanWallet_name, "f");
    }
    get icon() {
        return __classPrivateFieldGet(this, _XamanWallet_icon, "f");
    }
    get chains() {
        return [XRPL_MAINNET, XRPL_TESTNET, XRPL_DEVNET, XAHAU_MAINNET, XAHAU_TESTNET];
    }
    get features() {
        return {
            'standard:connect': {
                version: '1.0.0',
                connect: __classPrivateFieldGet(this, _XamanWallet_connect, "f"),
            },
            'standard:disconnect': {
                version: '1.0.0',
                disconnect: __classPrivateFieldGet(this, _XamanWallet_disconnect, "f"),
            },
            'standard:events': {
                version: '1.0.0',
                on: __classPrivateFieldGet(this, _XamanWallet_on, "f"),
            },
            'xrpl:signTransaction': {
                version: '1.0.0',
                signTransaction: __classPrivateFieldGet(this, _XamanWallet_signTransaction, "f"),
            },
            'xrpl:signAndSubmitTransaction': {
                version: '1.0.0',
                signAndSubmitTransaction: __classPrivateFieldGet(this, _XamanWallet_signAndSubmitTransaction, "f"),
            },
        };
    }
    get accounts() {
        return __classPrivateFieldGet(this, _XamanWallet_accounts, "f");
    }
    constructor(apiKey) {
        _XamanWallet_instances.add(this);
        _XamanWallet_name.set(this, 'Xaman');
        _XamanWallet_icon.set(this, 'data:image/png;base64,');
        _XamanWallet_accounts.set(this, []);
        _XamanWallet_client.set(this, void 0);
        _XamanWallet_sdk.set(this, void 0);
        _XamanWallet_listeners.set(this, {});
        _XamanWallet_connect.set(this, async ({ silent = false } = {}) => {
            if (!silent) {
                await __classPrivateFieldGet(this, _XamanWallet_client, "f").authorize();
            }
            const state = __classPrivateFieldGet(this, _XamanWallet_client, "f").state();
            if (!state)
                return { accounts: [] };
            const resolvedState = await state;
            if (!resolvedState)
                return { accounts: [] };
            __classPrivateFieldSet(this, _XamanWallet_accounts, [new XRPLWalletAccount(resolvedState.me.account)], "f");
            __classPrivateFieldSet(this, _XamanWallet_sdk, resolvedState.sdk, "f");
            __classPrivateFieldGet(this, _XamanWallet_instances, "m", _XamanWallet_emit).call(this, 'change', { accounts: this.accounts });
            return {
                accounts: this.accounts,
            };
        });
        _XamanWallet_disconnect.set(this, async () => {
            __classPrivateFieldGet(this, _XamanWallet_client, "f").logout();
        });
        _XamanWallet_signTransaction.set(this, async ({ tx_json, account, network }) => {
            const createdPayload = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.create({
                txjson: tx_json,
                options: { submit: false, force_network: __classPrivateFieldGet(this, _XamanWallet_instances, "m", _XamanWallet_getXamanNetworkNameFromChain).call(this, network) },
            });
            if (!createdPayload)
                throw new Error('Invalid Payload Parameter');
            const uuid = createdPayload.uuid;
            const subscription = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.subscribe(uuid);
            subscription.websocket.onmessage = (message) => {
                // resolve promise when receive signed message
                if (message.data.toString().match(/signed/)) {
                    const json = JSON.parse(message.data.toString());
                    subscription.resolve(json.signed);
                }
            };
            await subscription.resolved;
            const result = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.get(uuid);
            if (!result || !result.response.hex)
                throw new Error('Invalid Payload Parameter');
            return {
                signed_tx_blob: result.response.hex,
            };
        });
        _XamanWallet_signAndSubmitTransaction.set(this, async ({ tx_json, account, network }) => {
            const createdPayload = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.create({
                txjson: tx_json,
                options: { submit: true, force_network: __classPrivateFieldGet(this, _XamanWallet_instances, "m", _XamanWallet_getXamanNetworkNameFromChain).call(this, network) },
            });
            if (!createdPayload)
                throw new Error('Invalid Payload Parameter');
            const uuid = createdPayload.uuid;
            const subscription = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.subscribe(uuid);
            subscription.websocket.onmessage = (message) => {
                // resolve promise when receive signed message
                if (message.data.toString().match(/"signed"/)) {
                    const json = JSON.parse(message.data.toString());
                    subscription.resolve(json.payload_uuidv4);
                }
            };
            const payloadId = (await subscription.resolved);
            const payload = await __classPrivateFieldGet(this, _XamanWallet_sdk, "f").payload.get(payloadId);
            if (!payload || !payload.response.txid || !payload.response.dispatched_to)
                throw new Error('Invalid Payload');
            const client = new Client(payload.response.dispatched_to);
            await client.connect();
            const transactionResponse = await client.request({
                command: 'tx',
                transaction: payload.response.txid,
                api_version: 1,
            });
            await client.disconnect();
            return {
                tx_json: transactionResponse.result,
                tx_hash: payload.response.txid,
            };
        });
        _XamanWallet_on.set(this, (event, listener) => {
            var _a;
            if (__classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event])
                (_a = __classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.push(listener);
            else
                __classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event] = [listener];
            return () => __classPrivateFieldGet(this, _XamanWallet_instances, "m", _XamanWallet_off).call(this, event, listener);
        });
        if (new.target === XamanWallet) {
            Object.freeze(this);
        }
        __classPrivateFieldSet(this, _XamanWallet_client, new XummPkce(apiKey), "f");
    }
}
_XamanWallet_name = new WeakMap(), _XamanWallet_icon = new WeakMap(), _XamanWallet_accounts = new WeakMap(), _XamanWallet_client = new WeakMap(), _XamanWallet_sdk = new WeakMap(), _XamanWallet_listeners = new WeakMap(), _XamanWallet_connect = new WeakMap(), _XamanWallet_disconnect = new WeakMap(), _XamanWallet_signTransaction = new WeakMap(), _XamanWallet_signAndSubmitTransaction = new WeakMap(), _XamanWallet_on = new WeakMap(), _XamanWallet_instances = new WeakSet(), _XamanWallet_emit = function _XamanWallet_emit(event, ...args) {
    var _a;
    (_a = __classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.forEach((listener) => listener.apply(null, args));
}, _XamanWallet_off = function _XamanWallet_off(event, listener) {
    var _a;
    __classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event] = (_a = __classPrivateFieldGet(this, _XamanWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.filter((existingListener) => listener !== existingListener);
}, _XamanWallet_getXamanNetworkNameFromChain = function _XamanWallet_getXamanNetworkNameFromChain(chain) {
    switch (chain) {
        case XRPL_MAINNET:
        case 'xrpl:mainnet':
            return 'MAINNET';
        case XRPL_TESTNET:
        case 'xrpl:testnet':
            return 'TESTNET';
        case XRPL_DEVNET:
        case 'xrpl:devnet':
            return 'DEVNET';
        case XAHAU_MAINNET:
        case 'xrpl:xahau-mainnet':
            return 'XAHAUMAINNET';
        case XAHAU_TESTNET:
        case 'xrpl:xahau-testnet':
            return 'XAHAUTESTNET';
        default:
            throw new Error('Invalid Chain');
    }
};
