"use strict";
var __classPrivateFieldGet = (this && this.__classPrivateFieldGet) || function (receiver, state, kind, f) {
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a getter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
    return kind === "m" ? f : kind === "a" ? f.call(receiver) : f ? f.value : state.get(receiver);
};
var __classPrivateFieldSet = (this && this.__classPrivateFieldSet) || function (receiver, state, value, kind, f) {
    if (kind === "m") throw new TypeError("Private method is not writable");
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a setter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot write private member to an object whose class did not declare it");
    return (kind === "a" ? f.call(receiver, value) : f ? f.value = value : state.set(receiver, value)), value;
};
var _CrossmarkWallet_instances, _CrossmarkWallet_name, _CrossmarkWallet_icon, _CrossmarkWallet_accounts, _CrossmarkWallet_listeners, _CrossmarkWallet_connect, _CrossmarkWallet_signTransaction, _CrossmarkWallet_signAndSubmitTransaction, _CrossmarkWallet_on, _CrossmarkWallet_emit, _CrossmarkWallet_off;
Object.defineProperty(exports, "__esModule", { value: true });
exports.CrossmarkWallet = void 0;
const base_1 = require("./base");
const core_1 = require("./core");
class CrossmarkWallet {
    get version() {
        return '1.0.0';
    }
    get name() {
        return __classPrivateFieldGet(this, _CrossmarkWallet_name, "f");
    }
    get icon() {
        return __classPrivateFieldGet(this, _CrossmarkWallet_icon, "f");
    }
    get chains() {
        return [core_1.XRPL_MAINNET];
    }
    get features() {
        return {
            'standard:connect': {
                version: '1.0.0',
                connect: __classPrivateFieldGet(this, _CrossmarkWallet_connect, "f"),
            },
            'standard:events': {
                version: '1.0.0',
                on: __classPrivateFieldGet(this, _CrossmarkWallet_on, "f"),
            },
            'xrpl:signTransaction': {
                version: '1.0.0',
                signTransaction: __classPrivateFieldGet(this, _CrossmarkWallet_signTransaction, "f"),
            },
            'xrpl:signAndSubmitTransaction': {
                version: '1.0.0',
                signAndSubmitTransaction: __classPrivateFieldGet(this, _CrossmarkWallet_signAndSubmitTransaction, "f"),
            },
        };
    }
    get accounts() {
        return __classPrivateFieldGet(this, _CrossmarkWallet_accounts, "f");
    }
    constructor() {
        _CrossmarkWallet_instances.add(this);
        _CrossmarkWallet_name.set(this, 'Crossmark');
        _CrossmarkWallet_icon.set(this, 'data:image/png;base64,');
        _CrossmarkWallet_accounts.set(this, []);
        _CrossmarkWallet_listeners.set(this, {});
        _CrossmarkWallet_connect.set(this, async ({ silent = false } = {}) => {
            let address;
            if (silent) {
                address = window.xrpl.crossmark.session.address;
            }
            else {
                const { response } = await window.xrpl.crossmark.async.signInAndWait();
                address = response.data.address;
            }
            if (!address)
                return { accounts: [] };
            __classPrivateFieldSet(this, _CrossmarkWallet_accounts, [new base_1.XRPLWalletAccount(address)], "f");
            __classPrivateFieldGet(this, _CrossmarkWallet_instances, "m", _CrossmarkWallet_emit).call(this, 'change', { accounts: this.accounts });
            return {
                accounts: this.accounts,
            };
        });
        _CrossmarkWallet_signTransaction.set(this, async ({ tx_json, account, network }) => {
            // Crossmark does not support changing the network when signing a transaction
            const { response } = await window.xrpl.crossmark.async.signAndWait(tx_json);
            return {
                signed_tx_blob: response.data.txBlob,
            };
        });
        _CrossmarkWallet_signAndSubmitTransaction.set(this, async ({ tx_json, account, network }) => {
            // Crossmark does not support changing the network when signing a transaction
            const { response } = await window.xrpl.crossmark.async.signAndSubmitAndWait(tx_json);
            return {
                tx_json: response.data.resp.result,
                tx_hash: response.data.resp.result.hash,
            };
        });
        _CrossmarkWallet_on.set(this, (event, listener) => {
            var _a;
            if (__classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event])
                (_a = __classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.push(listener);
            else
                __classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event] = [listener];
            return () => __classPrivateFieldGet(this, _CrossmarkWallet_instances, "m", _CrossmarkWallet_off).call(this, event, listener);
        });
        if (new.target === CrossmarkWallet) {
            Object.freeze(this);
        }
    }
}
exports.CrossmarkWallet = CrossmarkWallet;
_CrossmarkWallet_name = new WeakMap(), _CrossmarkWallet_icon = new WeakMap(), _CrossmarkWallet_accounts = new WeakMap(), _CrossmarkWallet_listeners = new WeakMap(), _CrossmarkWallet_connect = new WeakMap(), _CrossmarkWallet_signTransaction = new WeakMap(), _CrossmarkWallet_signAndSubmitTransaction = new WeakMap(), _CrossmarkWallet_on = new WeakMap(), _CrossmarkWallet_instances = new WeakSet(), _CrossmarkWallet_emit = function _CrossmarkWallet_emit(event, ...args) {
    var _a;
    (_a = __classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.forEach((listener) => listener.apply(null, args));
}, _CrossmarkWallet_off = function _CrossmarkWallet_off(event, listener) {
    var _a;
    __classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event] = (_a = __classPrivateFieldGet(this, _CrossmarkWallet_listeners, "f")[event]) === null || _a === void 0 ? void 0 : _a.filter((existingListener) => listener !== existingListener);
};
