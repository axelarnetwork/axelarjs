...
import { AxelarSigningStargateClient } from "../stargateClient";
import { AXELAR_RPC_URL } from "./constants";

describe("query client", () => {
  test("query erc20Tokens", async () => {
    const client = await AxelarSigningStargateClient.connect(AXELAR_RPC_URL);

    const erc20Tokens = await client.query?.evm.eRC20Tokens({
      chain: "fantom",
